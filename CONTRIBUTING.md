...
then run `npm run deploy`. Obsidian does not hot-reload, so toggle the plugin
off and on, or reload the vault, to pick up a new build.
...
- **Put new logic in a pure module** (`src/otd.ts`, `src/dates.ts`,
  ...
  `obsidian` can't be unit-tested, so the pure modules are where the tests live.
- `npm run format` (oxfmt) handles formatting. Don't hand-align or hand-wrap
  prose.
